# scripts/opcodes/PRINCEv2/prince_v2_optimized3.js

Source: https://github.com/BitVM/bitvm-js/blob/b931a6711ab332fd5923e708c869bed02e39984e/scripts/opcodes/PRINCEv2/prince_v2_optimized3.js

Retrieved: 2026-08-24T21:53:24Z

Extraction note: Exact text file from immutable Git commit b931a6711ab332fd5923e708c869bed02e39984e.
...
const SIZE_MEMORY = SIZE_STATE + SIZE_KEY + SIZE_SHIFT_TABLE + SIZE_SBOX_TABLE + SIZE_SBOX_INV_TABLE + SIZE_XOR_TABLE + SIZE_AND_TABLE * 8 + SIZE_SBOX_XOR_TABLES
...
const ADDR_SBOX_TABLE = 48
const ADDR_AND_M0_SHIFT_TABLE = 64
const ADDR_AND_M1_SHIFT_TABLE = 80
const ADDR_AND_M2_SHIFT_TABLE = 96
const ADDR_AND_M3_SHIFT_TABLE = 112
const ADDR_SBOX_INV_TABLE = 128
const ADDR_SHIFT_TABLE = 144
const ADDR_AND_M0_TABLE = 160
const ADDR_AND_M1_TABLE = 176
const ADDR_AND_M2_TABLE = 192
const ADDR_AND_M3_TABLE = 208
const ADDR_XOR_TABLE = 224
const ADDR_SBOX_XOR_2_TABLE = 480
const ADDR_SBOX_XOR_3_TABLE = 496
const ADDR_SBOX_XOR_4_TABLE = 512
const ADDR_SBOX_XOR_8_TABLE = 528
const ADDR_SBOX_XOR_9_TABLE = 544
const ADDR_SBOX_XOR_14_TABLE = 560
const AND_VALUE_BIAS = 5
const XOR_SHIFT_BIAS = [-2, -2, 6, 2]
...
const push_and_table_m0 = loop(16, i => (M[0] & i) + AND_VALUE_BIAS).reverse()
const push_and_table_m1 = loop(16, i => (M[1] & i) + AND_VALUE_BIAS).reverse()
const push_and_table_m2 = loop(16, i => (M[2] & i) + AND_VALUE_BIAS).reverse()
const push_and_table_m3 = loop(16, i => (M[3] & i) + AND_VALUE_BIAS).reverse()

const push_and_shift_table_m0 = loop(16, i => (M[0] & i) * 16 + ADDR_XOR_TABLE - 1 + XOR_SHIFT_BIAS[0]).reverse()
const push_and_shift_table_m1 = loop(16, i => (M[1] & i) * 16 + ADDR_XOR_TABLE - 1 + XOR_SHIFT_BIAS[1]).reverse()
const push_and_shift_table_m2 = loop(16, i => (M[2] & i) * 16 + ADDR_XOR_TABLE - 1 + XOR_SHIFT_BIAS[2]).reverse()
const push_and_shift_table_m3 = loop(16, i => (M[3] & i) * 16 + ADDR_XOR_TABLE - 1 + XOR_SHIFT_BIAS[3]).reverse()

const push_sbox_table = PRINCE_SBOX.slice().reverse()
const push_sbox_inv_table = PRINCE_SBOX_INVERSE.slice().reverse()
const push_sbox_xor_2_table = loop(16, i => PRINCE_SBOX[i ^ 2]).reverse()
const push_sbox_xor_3_table = loop(16, i => PRINCE_SBOX[i ^ 3]).reverse()
const push_sbox_xor_4_table = loop(16, i => PRINCE_SBOX[i ^ 4]).reverse()
const push_sbox_xor_8_table = loop(16, i => PRINCE_SBOX[i ^ 8]).reverse()
const push_sbox_xor_9_table = loop(16, i => PRINCE_SBOX[i ^ 9]).reverse()
const push_sbox_xor_14_table = loop(16, i => PRINCE_SBOX[i ^ 14]).reverse()

const push_tables = _ => [
    push_sbox_xor_14_table,
    push_sbox_xor_9_table,
    push_sbox_xor_8_table,
    push_sbox_xor_4_table,
    push_sbox_xor_3_table,
    push_sbox_xor_2_table,
    push_xor_table,
    push_and_table_m3,
    push_and_table_m2,
    push_and_table_m1,
...
const op_sbox_xor_constant = (constant, scratch = 0) => {
    switch (constant) {
        case 2: return [ADDR_SBOX_XOR_2_TABLE - 1 + scratch, OP_ADD, OP_PICK];
        case 3: return [ADDR_SBOX_XOR_3_TABLE - 1 + scratch, OP_ADD, OP_PICK];
        case 4: return [ADDR_SBOX_XOR_4_TABLE - 1 + scratch, OP_ADD, OP_PICK];
        case 8: return [ADDR_SBOX_XOR_8_TABLE - 1 + scratch, OP_ADD, OP_PICK];
        case 9: return [ADDR_SBOX_XOR_9_TABLE - 1 + scratch, OP_ADD, OP_PICK];
        case 14: return [ADDR_SBOX_XOR_14_TABLE - 1 + scratch, OP_ADD, OP_PICK];
        default: return [op_xor_constant(constant, scratch), op_sbox(scratch)];
    }
}
...
const prince_MHatMultiply = (base, useMHat0, pre_action = null, scratch = 0) => {
    ...
    return [
        ...
        loop(4, r => {
            ...
            const c0 = C_idx(r, 0), c1 = C_idx(r, 1), c2 = C_idx(r, 2), c3 = C_idx(r, 3);
            return [
                op_and_m(c0, s - 0),   
                OP_SWAP, op_and_m_shift(c1, s - 0), op_xor_shifted(s - 1 - AND_VALUE_BIAS - XOR_SHIFT_BIAS[c1]),                
                OP_SWAP, op_and_m_shift(c2, s - 1), op_xor_shifted(s - 2 - XOR_SHIFT_BIAS[c2]),
                OP_SWAP, op_and_m_shift(c3, s - 2), op_xor_shifted(s - 3 - XOR_SHIFT_BIAS[c3]),                  
                OP_TOALTSTACK,
            ];
        }),
    ...
    loop(4, i => {
        ...
        return [
            prince_m_layer(idx => [
                ...
                op_xor_shifted(),
                op_sbox_xor_constant(RC[r - 1][idx])
            ]),
            prince_shiftRow(false)
        ]
    }),
